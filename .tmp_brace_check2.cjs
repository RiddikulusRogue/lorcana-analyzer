...
for (let i=0;i<s.length;i++) {
  ...
  if (state==='code') {
    ...
    if (c==='}'||c===')'||c===']') {
      ...
      if (c==='}' && last && last.c==='${') {
        stack.pop(); state='template'; continue;
      }
      const map={'}':'{',')':'(',']':'['};
      const popped=stack.pop();
      if (!popped || popped.c!==map[c]) { console.log('Mismatch at',i,'got',c,'stackTop',popped); process.exit(0);} 
    }
  } else if (state==='line') {
    if (c==='\n') state='code';
  } else if (state==='block') {
    ...
    if (c==='$' && n==='{') { stack.push({c:'${',i}); state='code'; i++; continue; }
  }
}
